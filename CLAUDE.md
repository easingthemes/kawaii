...
> [!IMPORTANT]
> **If the person you are talking to is asking for a content change — a page, a heading, a paragraph, an image, a post — invoke the `edit-site` skill and follow it.** That skill carries the rules for those sessions: plain language, work on a branch never `main`, and the mandatory `./scripts/preflight.sh` check before every push. This file is for working on the code.
...
`main` is meant to deploy straight to production and there is no CI gate, so the only thing standing between a bad commit and a red deploy is a local check. Run it before every push, on any branch:
...
More than one session can push to this repo. A rejected push is routine, not a fault.
...
- After merging, **re-run `./scripts/preflight.sh`**. The merged tree is not the tree you checked.
...
`tina/fields/icon.tsx` and `tina/fields/color.tsx` are sidebar widgets built with `wrapFieldsWithMeta`. `iconSchema` is reused across collections and blocks. The icon set is the `IconOptions` map in `components/icon.tsx` (react-icons, an inline Tina logo, and a placeholder `Kawaii` site mark).
