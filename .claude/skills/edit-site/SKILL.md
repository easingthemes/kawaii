---
name: edit-site
description: Change the text, images or pages of the Kawaii website on someone's behalf, then get the change onto a preview link safely. Use for any request to edit, add, remove or reword site content — a page, a heading, a paragraph, a photo, a blog post — especially when the person asking is not a developer and is working from a phone. Covers the required pre-push build check and what to report back afterwards.
---

# Editing the Kawaii site

The site's owner edits it themselves through Claude Code, often on a phone, and
is **not a technical person**. Dragan (`vebkurs@gmail.com`) is the developer.

Your job is to make the change they ask for, prove it doesn't break the site,
put it on a preview link, and tell them what to click. Nothing else.
...
| 2 | **the machine is dirty** | says nothing about the change. Never report this as their mistake. |
...
**Exit 2 is not a failure of their edit.** It means something on this machine got
...
**Answer in the language they write in.** The Serbian examples below are there
because that is the language this setup was first used in — match whatever
language you are written to in.

**These words must never appear in a message to them.** Not explained, not in
...
Say the thing that happened to *their site* instead:
...
| this is ready to merge | (never say it — publishing is not their decision and not the goal) |
...
- **Never paste an error at them**, not even one line of it. Say what it means
  for their site and what you are doing about it.
- **Never ask them to choose between technical options.** Pick the safe one.
- **Never make them feel at fault.** If something broke, it broke — do not
  explain that their text, their image or their wording caused it.
- **Do not narrate your work.** They want the result, not the steps.

Good: *"Promenila sam naslov na strani O nama. Evo kako sada
...
1. **Understand what they want.** If it's ambiguous, ask one plain question.
   If they send a photo or text, use it as-is — don't rewrite their words.
...
3. **Never work on `main`.** Branch from `main`, with a short name describing
   the change, and push that. If a branch is already being used for the site's
   current round of edits — the one whose preview they have been looking at —
   keep using it rather than cutting a fresh one per edit, so their change shows
   up alongside everyone else's on the same preview.
...
6. **Push the branch**, never `main`. Other people may work on this repo at the
...
- **Vercel MCP tools** if available. `get_deployment` on the branch alias, and
  ...
  `ERROR` means read `get_deployment_build_logs`. The Vercel project is
  `kawaii`; confirm the team slug from the dashboard the first time.
- **GitHub commit status** otherwise — pass/fail only, no log.
- If neither is reachable, say you could not verify the build, and give them
  the link anyway with that caveat.
...
```
https://kawaii-git-<branch>-<team>.vercel.app
```
...
the guessed one 404s. The preview is public — no login needed, they can just
open it on a phone.

A build takes 1–3 minutes. Do not tell them it's ready before it is.

**If the build fails after you pushed:** fix it and push again. Two attempts.
If it's still red, stop, tell them plainly that it needs Dragan, and say what
is broken. Don't keep pushing.

## Out of scope — stop and hand over

Tell them this needs Dragan, and do not attempt it:
...
- anything touching `tina/`, `components/`, `app/`, `next.config.ts`,
  `package.json`
- merging to `main` / publishing to kawaii.vercel.app
- domains, DNS, Vercel settings, environment variables

They can *ask* for these — just don't build them in a session that was meant to
be a content edit.

## Other people are working too

Dragan and other Claude sessions may push to the same branches while you work. This
is normal. Expect it, and never treat it as an error to report to them.
...
- **If the merge touches files you just edited**, read both sides. Keep both
  changes unless they genuinely contradict; if they do, keep the other side and
  say so to Dragan — not to the site's owner.
...
- Pushing to `main` deploys straight to kawaii.vercel.app. There is no staging
