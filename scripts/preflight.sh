#!/usr/bin/env bash
#
# Pre-push check: runs the same production build a deploy runs, so a broken commit never
# reaches the repo. Safe to run by hand at any time.
#
...
#     disk, a killed process — none of those would happen on a clean build server, and calling
#     them "the error the deploy would show" is wrong exactly when someone is most
...
step "Build (schema, types and every page — same as a deploy; port $PORT)"
if ! NODE_ENV=production npx tinacms build --local --skip-cloud-checks --noTelemetry \
  ...
  echo
  echo 'VERDICT: DO NOT PUSH — this is the error the deploy would show.'
